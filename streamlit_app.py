...
# Try importing the scoring logic and show any error ON SCREEN
try:
    from credit_scoring import evaluate_customer
except Exception as e:
    st.error("❌ Error importing credit_scoring module. Details:")
    st.code(str(e))
    st.stop()
...
# -------------------------------------------------------------------
#                    STREAMLIT CONFIG & DEFAULT STATE
# -------------------------------------------------------------------
st.set_page_config(
...
st.write("Fill customer data, upload bank statement for new customers, and get a credit decision.")

# Defaults for behaviour (used by bank analyser)
if "avg_days_to_pay" not in st.session_state:
    st.session_state["avg_days_to_pay"] = 45
if "bounced_cheques_last_12m" not in st.session_state:
    st.session_state["bounced_cheques_last_12m"] = 0
if "past_default_flag" not in st.session_state:
    st.session_state["past_default_flag"] = "No"
...
with c1:
    cin_no = st.text_input("CIN", key="cin_no")
with c2:
    customer_name = st.text_input("Customer Name (as per MCA)", key="customer_name")
with c3:
    fetch_mca = st.button("Fetch from MCA (Mock)")

p1, p2 = st.columns(2)
with p1:
    pan_no = st.text_input("PAN No", key="pan_no")
with p2:
    gst_no = st.text_input("GST No", key="gst_no")

a1, a2, a3 = st.columns(3)
with a1:
    registered_address = st.text_area("Registered Address", height=80, key="registered_address")
with a2:
    city = st.text_input("City", key="city")
    state = st.text_input("State", key="state")
    pincode = st.text_input("Pincode", key="pincode")
with a3:
    directors_details = st.text_area("Director(s) Name & Address", height=80, key="directors_details")

# Handle MCA mock fetch
if fetch_mca and cin_no:
    ...
    if data:
        ...
        st.session_state["registered_address"] = data["registered_address"]
        st.session_state["city"] = data["city"]
        st.session_state["state"] = data["state"]
        st.session_state["pincode"] = data["pincode"]
        st.session_state["directors_details"] = data["directors_details"]
        st.session_state["latest_fy"] = data["latest_fy"]
        st.session_state["authorised_capital"] = float(data["authorised_capital"])
        st.session_state["paidup_capital"] = float(data["paidup_capital"])
        st.session_state["company_age"] = float(data["company_age"])
        st.session_state["revenue_growth"] = float(data["revenue_growth"])
        st.session_state["profit_growth"] = float(data["profit_growth"])
        st.session_state["employee_count"] = int(data["employee_count"])
        st.session_state["ownership_structure"] = data["ownership_structure"]
        st.session_state["key_members"] = data["key_members"]
        st.success("Mock MCA data loaded for this CIN.")
    else:
        st.warning("No mock data found for this CIN. Use test CINs shown below.")
...
with ki1:
    latest_fy = st.text_input("Financial Year (e.g. 2023-24)", key="latest_fy")
    company_age = st.number_input("Company Age (years)", min_value=0.0, step=0.5, key="company_age")
with ki2:
    authorised_capital = st.number_input("Authorised Capital (₹)", min_value=0.0, step=100000.0, key="authorised_capital")
    revenue_growth = st.number_input("Revenue Growth (3 yrs, %)", step=0.1, key="revenue_growth")
with ki3:
    paidup_capital = st.number_input("Paid-up Capital (₹)", min_value=0.0, step=100000.0, key="paidup_capital")
    profit_growth = st.number_input("Profit Growth (3 yrs, %)", step=0.1, key="profit_growth")

ki4, ki5 = st.columns(2)
with ki4:
    employee_count = st.number_input("Employee Count", min_value=0, step=1, key="employee_count")
with ki5:
    ownership_structure = st.text_area("Ownership & Shareholding Structure", height=80, key="ownership_structure")
key_members = st.text_area("Key Members / Board of Directors", height=80, key="key_members")
...
if analyze_bank_btn:
    if bank_file is None:
        st.error("Please upload a bank statement file first.")
    else:
        try:
            ...
            st.info(
                f"Bank analysis → Bounces: {bank_res['bounced_cheques_last_12m']}, "
                f"negative balance days: {bank_res['negative_days']}, "
                f"inflow/outflow ratio: {bank_res['cashflow_ratio']}, "
                f"avg days to pay (proxy): {bank_res['avg_days_to_pay']}."
            )
        except Exception as e:
...
with pb2:
    bounced_cheques_last_12m = st.number_input(
        ...
        key="bounced_cheques_last_12m",
    )
with pb3:
    past_default_flag_str = st.selectbox(
        "Any Past Default / Write-off?",
        ["No", "Yes"],
        index=0 if st.session_state["past_default_flag"] == "No" else 1,
        key="past_default_flag",
    )
...
with b3:
    top_5_customer_share = st.number_input(
        "Top 5 Customers' Share of Sales (%)", min_value=0.0, max_value=100.0, step=1.0, value=50.0
    )
with b4:
...
if st.button("Evaluate Credit Worthiness"):
    ...
    scoring_input = {
        ...
        "past_default_flag": (past_default_flag_str == "Yes"),
    ...
    st.write(f"**Name:** {st.session_state.get('customer_name') or '—'}")
    st.write(f"**CIN:** {st.session_state.get('cin_no') or '—'}")
    st.write(f"**PAN:** {st.session_state.get('pan_no') or '—'}")
    st.write(f"**GST:** {st.session_state.get('gst_no') or '—'}")
    st.write(f"**Registered Address:** {st.session_state.get('registered_address') or '—'}")
    st.write(
        f"**Location:** {st.session_state.get('city') or ''} "
        f"{st.session_state.get('state') or ''} {st.session_state.get('pincode') or ''}"
    )
    st.write("**Director(s):**")
    st.code(st.session_state.get("directors_details") or "—")

    st.subheader("🏢 Company Profile & Key Indicators")
    st.write(f"**Financial Year:** {st.session_state.get('latest_fy') or '—'}")
    st.write(f"**Authorised Capital:** {st.session_state.get('authorised_capital') or '—'}")
    st.write(f"**Paid-up Capital:** {st.session_state.get('paidup_capital') or '—'}")
    st.write(f"**Company Age:** {st.session_state.get('company_age') or '—'} years")
    st.write(f"**Revenue Growth (3 yrs):** {st.session_state.get('revenue_growth') or '—'} %")
    st.write(f"**Profit Growth (3 yrs):** {st.session_state.get('profit_growth') or '—'} %")
    st.write(f"**Employee Count:** {st.session_state.get('employee_count') or '—'}")
    st.write("**Ownership / Shareholding:**")
    st.code(st.session_state.get("ownership_structure") or "—")
    st.write("**Key Members / Board:**")
    st.code(st.session_state.get("key_members") or "—")
